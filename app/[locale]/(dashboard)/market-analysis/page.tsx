  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        {[
          { label: t("confidence"), value: `${sigma.confidence ?? 0}%` },
          { label: t("positionSize"), value: fmtUSDC(sigma.size_usd ?? 0) },
          { label: t("entryTarget"), value: `${Math.round(((sigma.entry_price ?? 0) as number) * 100)}¢` },
        ].map((m) => (
          <div key={m.label}>
            <span
              style={{
    ...
    <div suppressHydrationWarning style={{ display: "flex", flexDirection: "column", gap: 0, height: "calc(100vh - 92px)" }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: BODY_SIZE, color: "rgba(255,255,255,0.30)" }}>
            {selectedMarket ? (
              <>
                ...
                {" "}
                {pipelineRunning ? (
                  <span style={{ color: "#ff9f0a" }}>— {t("runningAnalysis")}</span>
                ) : pipelineResult ? (
                  <span style={{ color: "#30d158" }}>— {t("analysisComplete")} ✓</span>
                ) : null}
              </>
            ) : (
              t("selectMarket")
      ...
      >
        ...
        >
          ...
          >
            {t("marketsClickToAnalyze", { count: markets.length })}
          </div>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
              ...
              <div style={{ flex: 1, overflowY: "auto" }}>
                ...
                {pipelineError && (
                  ...
                  >
                    ✗ {t("pipelineError", { error: pipelineError })}
                  </div>
                )}
              </div>
